...
const Banner = ({ type, message }) => {
  ...
  return (
    <div className={`px-4 py-3 rounded-lg border text-sm ${styles}`}>{message}</div>
  );
};
...
const PersonalInfoSection = ({ profile, onUpdated }) => {
  ...
  const handleSave = async () => {
    if (!name.trim()) { setBanner({ type: 'error', message: 'Name is required.' }); return; }
    setLoading(true);
    setBanner(null);
    try {
      const updated = await updateProfileApi({ name: name.trim(), phone: phone.trim() || null });
      onUpdated(updated);
      setBanner({ type: 'success', message: 'Profile updated successfully.' });
    } catch (err) {
      setBanner({ type: 'error', message: err?.response?.data?.error || 'Could not save changes.' });
    } finally {
      setLoading(false);
    }
  };
...
const EmailSection = ({ profile, onLogout }) => {
  ...
  const handleChange = async () => {
    if (!newEmail.trim() || !password) {
      setBanner({ type: 'error', message: 'New email and current password are required.' });
      return;
    }
    setLoading(true);
    setBanner(null);
    try {
      ...
      setBanner({ type: 'error', message: err?.response?.data?.error || 'Could not update email.' });
      setLoading(false);
    }
  };
  ...
  if (emailSent) {
    return (
      <Section title="Email address" description="Used to log in and receive notifications.">
        <div className="text-center py-4">
          <div className="w-12 h-12 rounded-full bg-[#445446]/10 flex items-center justify-center mx-auto mb-3">
            <svg className="w-6 h-6 text-[#445446]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
            </svg>
          </div>
          <p className="text-sm font-semibold text-[#1F2933] mb-1">Verify your new email</p>
          ...
          <button onClick={onLogout} className="text-sm text-[#445446] font-medium hover:underline">
    ...
    <Section title="Email address" description="Used to log in and receive notifications.">
      <div className="space-y-4">
        <div className="flex items-center justify-between px-4 py-3 bg-[#F5F7F5] rounded-lg border border-[#E4E7E4]">
          <span className="text-sm text-[#1F2933] font-medium">{profile.email}</span>
          {profile.is_verified
        ...
        {!showForm ? (
          ...
          <div className="space-y-4 pt-1">
            <div className="px-4 py-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
              Changing your email will sign you out. You must verify the new address before logging back in.
            </div>
            <Banner type={banner?.type} message={banner?.message} />
            ...
            <div className="flex gap-3 justify-end pt-1">
              <button
                onClick={() => { setShowForm(false); setBanner(null); setNewEmail(''); setPassword(''); }}
...
const ChangePasswordSection = ({ onLogout }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setBanner({ type: 'success', message: 'Password changed. You have been signed out of all other sessions.' });
      setCurrent(''); setNext(''); setConfirm('');
      // Sign out current session too — user should re-login with new password
      setTimeout(onLogout, 2000);
    } catch (err) {
      setBanner({ type: 'error', message: err?.response?.data?.error || 'Could not change password.' });
      setLoading(false);
    }
  };

  return (
    <Section title="Change password" description="Requires your current password for confirmation.">
      <div className="space-y-4">
        <Banner type={banner?.type} message={banner?.message} />
        ...
        <Field label="New password">
          <Input type="password" value={next} onChange={(e) => setNext(e.target.value)} placeholder="Minimum 6 characters" />
        </Field>
...
const DeleteAccountSection = ({ onLogout }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || 'Could not delete account. Please try again.');
      setLoading(false);
    }
  };
        ...
        {!showConfirm ? (
          <div className="space-y-4">
            <div className="text-sm text-gray-600 leading-relaxed space-y-2">
              ...
              <p className="text-xs text-gray-400 mt-2">
                Booking records are retained in anonymised form for accounting purposes
                as required by law (GDPR Art. 17(3)(b)).
              </p>
            </div>
            <button
          ...
          <div className="space-y-4">
            <div className="px-4 py-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
              This action is <strong>permanent and irreversible</strong>. All your personal data will be erased immediately.
            </div>
