...
const SearchIcon = ({ active }) => (
  <svg className={`w-5 h-5 flex-shrink-0 ${active ? 'text-[#445446]' : 'text-current'}`}
...
const LogoutIcon = () => (
  <svg className="w-5 h-5 flex-shrink-0 text-current" fill="none" viewBox="0 0 24 24"
...
const ParentDashboard = () => {
  ...
  const activeSection = VALID_SECTIONS.includes(lastSegment) ? lastSegment : 'browse';
    ...
    <div className="min-h-screen bg-[#F5F7F5] flex">
      ...
      <aside className="fixed inset-y-0 left-0 w-64 bg-white border-r border-[#E4E7E4] flex flex-col z-10">
        ...
        <div className="h-16 flex items-center px-6 border-b border-[#E4E7E4] gap-2.5">
          <img src="/assets/images/Sage-Nest_Final.svg" alt="Sage Nest" className="h-8"
        ...
        <nav className="flex-1 py-4 px-3 space-y-0.5 overflow-y-auto">
          {NAV_ITEMS.map(({ key, label, Icon }) => {
            ...
            return (
              <Link key={key} to={`/dashboard/parent/${key}`}
