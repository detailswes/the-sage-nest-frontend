...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-700">
      ...
      {t("parentDetail.badge.suspended")}
    </span>
  );
};
...
const AdminParentDetailSection = () => {
  ...
  const bookingCount = parent._count?.bookings_as_parent ?? bookings.length;
  const TABS = [
    { key: "overview", label: t("parentDetail.tabs.overview"),  shortLabel: t("parentDetail.tabs.overview") },
    { key: "bookings", label: `${t("parentDetail.tabs.bookings")} (${bookingCount})`, shortLabel: `${t("parentDetail.tabs.bookings")} (${bookingCount})` },
    { key: "consents", label: t("parentDetail.tabs.consents"),  shortLabel: t("parentDetail.tabs.consentsShort", { defaultValue: "Consents" }) },
    { key: "activity", label: t("parentDetail.tabs.activity"),  shortLabel: t("parentDetail.tabs.activity") },
  ];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
        ...
        <div className="min-w-0 space-y-0">
          ...
          {/* Identity card */}
          <div className="bg-white rounded-2xl border-2 border-[#c5ceba] px-5 py-5 mb-5">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-500 mt-0.5 break-all">{parent.email}</p>
                {parent.phone && (
          ...
          <div className="bg-white rounded-2xl border-2 border-[#c5ceba] overflow-hidden">
            ...
            {/* Tab bar — 2×2 grid on mobile, horizontal row on lg+ */}
            <div className="border-b border-[#c5ceba]">
              {/* Mobile: 2×2 grid */}
              <div className="lg:hidden grid grid-cols-2 gap-px bg-[#c5ceba]">
                {TABS.map(({ key, shortLabel }) => (
                  <button
                    key={key}
                    onClick={() => setActiveTab(key)}
                    className={`py-3 px-3 text-xs font-medium transition-colors text-center ${
                      activeTab === key
                        ? "bg-[#445446] text-white"
                        : "bg-white text-gray-400 hover:text-[#5e6d5b] hover:bg-[#f9faf8]"
                    }`}
                  >
                    {shortLabel}
                  </button>
            ...
            <div className="p-5 sm:p-6 space-y-5">
              ...
              {activeTab === "overview" && (
                <>
                  {/* Contact info */}
                  <div>
                    <SectionLabel>{t("parentDetail.overview.contactTitle")}</SectionLabel>
                    <div className="rounded-xl border border-[#c5ceba] divide-y divide-[#dfe2d7] overflow-hidden">
                      {[
                        { label: t("parentDetail.overview.email"),
                          value: <span className="break-all">{parent.email}</span> },
                        { label: t("parentDetail.overview.phone"),
                          value: parent.phone || "—" },
                        { label: t("parentDetail.overview.emailVerified"),
                          value: (
                        ...
                        <div key={label} className="flex gap-3 px-4 py-2.5 text-sm">
                          ...
                          <span className="text-[#1F2933] min-w-0">{value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Account info */}
                  <div>
                    <SectionLabel>{t("parentDetail.overview.accountTitle")}</SectionLabel>
                    <div className="rounded-xl border border-[#c5ceba] divide-y divide-[#dfe2d7] overflow-hidden">
                      {[
                        { label: t("parentDetail.overview.status"),
                          value: <StatusBadge status={status} /> },
                        { label: t("parentDetail.overview.registered"),
                          value: formatDate(parent.created_at) },
                        { label: t("parentDetail.overview.totalBookings"),
                          value: parent._count?.bookings_as_parent ?? "—" },
                      ].map(({ label, value }) => (
                        <div key={label} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                          <span className="text-gray-400 w-36 flex-shrink-0">{label}</span>
                          <span className="text-[#1F2933] font-medium min-w-0">{value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {activeTab === "bookings" && (
                bookingsLoading ? (
                  <div className="flex items-center gap-2 py-4">
                    ...
                    <span className="text-xs text-gray-400">{t("parentDetail.bookingsTab.loading")}</span>
                  </div>
                ) : bookings.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {bookings.map((b) => {
                      const { primary, utc } = formatBookingTime(b.scheduled_at, null);
                      return (
                        <button
                          key={b.id}
                        ...
                        >
                          <div className="min-w-0">
                            <p className="text-xs font-medium text-[#1F2933] truncate">
                              #{b.id} · {b.service?.title || t("parentDetail.bookingsTab.sessionFallback")}
                            </p>
              ...
              {activeTab === "consents" && (() => {
                ...
                if (!lc) return (
                  <div className="flex flex-col items-center justify-center py-12">
                    <p className="text-sm text-gray-400">{t("parentDetail.consents.noData")}</p>
                  </div>
                );
                ...
                const ComplianceBadge = ({ ok }) =>
                  ok ? (
                    ...
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-600 flex-shrink-0">
                ...
                return (
                  <div className="space-y-5">
                    ...
                    {/* T&C history */}
                    <div className="bg-[#dfe2d7]/20 rounded-xl border border-[#c5ceba] px-4 py-4">
                      <SectionLabel>{t("parentDetail.consents.tcHistory")}</SectionLabel>
                      ...
                      {lc.tc_acceptances.length === 0 && !lc.tc_at_registration ? (
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-xs min-w-[280px]">
                            <thead><tr className="border-b border-[#c5ceba]">
                              {[t("parentDetail.consents.colVersion"), t("parentDetail.consents.colAcceptedOn"), t("parentDetail.consents.colSource")].map((h) => (
                                <th key={h} className="text-left text-[#5e6d5b] font-semibold pb-2 pr-4 whitespace-nowrap">{h}</th>
                              ))}
                            </tr></thead>
                            <tbody className="divide-y divide-[#dfe2d7]">
              ...
              {activeTab === "activity" && (
                auditLoading ? (
                  <div className="flex items-center gap-2 py-4">
                    <div className="w-4 h-4 rounded-full border-2 border-[#445446] border-t-transparent animate-spin" />
                    <span className="text-xs text-gray-400">{t("parentDetail.activityTab.loading")}</span>
                  </div>
                ) : auditLog.length === 0 ? (
                  ...
                  <ol className="relative border-l border-[#E4E7E4] ml-2 space-y-4">
                    {auditLog.map((entry) => (
                      <li key={entry.id} className="ml-4">
                        ...
                        {entry.note && (
                          <p className="text-xs text-gray-500 mt-1 bg-gray-50 border border-[#c5ceba] rounded-lg px-3 py-2">
                            {entry.note}
                          </p>
                        )}
                      </li>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border-2 border-[#c5ceba] p-5">
            <SectionLabel>{t("parentDetail.statusActions.title")}</SectionLabel>
            <div className="flex flex-col gap-2">
              {status !== "ACTIVE" && (
                <button
                  onClick={() => setShowConfirm("activate")}
                  disabled={anyActionLoading}
                  className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold border border-green-200 text-green-700 bg-green-50 hover:bg-green-100 transition-colors disabled:opacity-50"
                >
                  {activating ? t("parentDetail.statusActions.activating") : t("parentDetail.statusActions.activate")}
                </button>
              )}
              {status !== "SUSPENDED" && (
                <button
                  ...
                  disabled={anyActionLoading}
                  className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold border border-orange-200 text-orange-700 bg-orange-50 hover:bg-orange-100 transition-colors disabled:opacity-50"
                >
                  {suspending ? t("parentDetail.statusActions.suspending") : t("parentDetail.statusActions.suspend")}
                </button>
              )}
            </div>
          </div>
          ...
          {/* Support tools */}
          <div className="bg-white rounded-2xl border-2 border-[#c5ceba] p-5">
            <SectionLabel>{t("parentDetail.supportTools.title")}</SectionLabel>
            <div className="flex flex-col gap-2">
              <button
              ...
              >
                {resettingPassword ? t("parentDetail.supportTools.sending") : t("parentDetail.supportTools.passwordReset")}
              </button>
              {!parent.is_verified && (
                <>
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border border-[#c5ceba] text-gray-600 hover:bg-[#dfe2d7]/50 disabled:opacity-50 transition-colors"
                  >
                    {resendingVerif ? t("parentDetail.supportTools.sending") : t("parentDetail.supportTools.resendVerification")}
                  </button>
                  <button
                  ...
                  >
                    {verifying ? t("parentDetail.supportTools.verifying") : t("parentDetail.supportTools.markVerified")}
                  </button>
                </>
              )}
            </div>
          ...
          {/* Danger zone */}
          <div className="bg-white rounded-2xl border-2 border-dashed border-red-200 p-5">
            <SectionLabel>{t("parentDetail.dangerZone.title")}</SectionLabel>
            <p className="text-xs text-gray-400 mb-3">{t("parentDetail.dangerZone.subtitle")}</p>
            <button
              onClick={() => { setGdprEmail(""); setShowGdprDelete(true); }}
              className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-medium border border-red-200 text-red-600 bg-red-50 hover:bg-red-100 transition-colors"
            >
                          ...
                          <tbody className="divide-y divide-[#F0F2F0]">
                            {suspendPreview.bookings.map((b) => (
                              <tr key={b.id}>
                                ...
                                <td className="px-3 py-2 text-gray-500 max-w-[120px] truncate">{b.serviceTitle}</td>
                ...
                return (
                  <>
                    <div className="bg-amber-50 border border-amber-300 rounded-lg px-4 py-3 mb-4 text-xs text-amber-800 space-y-2">
                      ...
                      {upcomingBookings.length > 0 && <p><span className="font-medium">{upcomingBookings.length} {t("parentDetail.gdpr.blockedUpcoming", { count: upcomingBookings.length })}</span>{t("parentDetail.gdpr.blockedUpcomingSuffix")}</p>}
                      {overdueUnresolved.length > 0 && <p><span className="font-medium">{overdueUnresolved.length} {t("parentDetail.gdpr.blockedOverdue", { count: overdueUnresolved.length })}</span>{t("parentDetail.gdpr.blockedOverdueSuffix")}</p>}
                      {pendingRefunds.length > 0 && <p><span className="font-medium">{pendingRefunds.length} {t("parentDetail.gdpr.blockedRefunds", { count: pendingRefunds.length })}</span>{t("parentDetail.gdpr.blockedRefundsSuffix")}</p>}
                      {openDisputes.length > 0 && <p><span className="font-medium">{openDisputes.length} {t("parentDetail.gdpr.blockedDisputes", { count: openDisputes.length })}</span>{t("parentDetail.gdpr.blockedDisputesSuffix")}</p>}
              ...
              return (
                <>
                  <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-3 mb-4 text-xs text-red-700 space-y-2">
                    ...
                    <p className="text-red-600 border-t border-red-200 pt-2">{t("parentDetail.gdpr.retainNote")}</p>
                  </div>
                  <p className="text-sm text-gray-600 mb-2">
                    {t("parentDetail.gdpr.typeEmail")}{" "}
                    <span className="font-medium text-[#1F2933] break-all">{parent.email}</span>
                  </p>
                  <input
                    type="email"
                    value={gdprEmail}
